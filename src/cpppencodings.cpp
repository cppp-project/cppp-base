...
namespace cppp{namespace base{namespace encoding
{
    c_int get_reiconv_version(void)
    {
        return (c_int)::_libiconv_version;
    }

    iconv_t iconv_open(c_string* tocode, c_string* fromcode)
    {
        return ::iconv_open((const char*)tocode, (const char*)fromcode);
    }

    c_size real_iconv(iconv_t cd, c_buffer* inbuf, c_size* inbytesleft, c_buffer* outbuf, c_size* outbytesleft)
    {
        return (c_size)::iconv(cd, (const char**)inbuf, (size_t*)inbytesleft, (char**)outbuf, (size_t*)outbytesleft);
    }

    c_int iconv_close(iconv_t cd)
    {
        return (c_int)::iconv_close(cd);
    }

    c_int iconv_string(c_string tocode, c_string fromcode, c_byte* start, c_byte* end, c_buffer* resultp, c_size* lengthp)
    {
    ...
    c_int iconvctl(iconv_t cd, c_int request, c_pointer argument)
    {
        return (c_int)::iconvctl(cd, (int)request, (void*)argument);
    }

    void iconvlist(c_int (*do_one)(c_uint namescount, c_string* names, c_pointer data), c_pointer data)
    {
        ::iconvlist((int(*)(unsigned int, const char* const*, void*))do_one, data);
    }

    c_string iconv_canonicalize(c_string name)
    {
        return (c_string)::iconv_canonicalize((const char*)name);
    }

    c_string locale_charset(void)
    {
    ...
    c_int decode(c_wstring encoding, c_buffer in, c_size in_size, c_wbuffer* out, c_size* out_length, c_bool strict)
    {
    ...
    c_int encode(c_wstring encoding, c_wbuffer in, c_size in_size, c_buffer *out, c_size *out_length, c_bool strict)
    {
