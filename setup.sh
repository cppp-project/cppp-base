...
if test -d './cppp-reiconv'
then
  text_out 32 "Success" "'./cppp-reiconv' is exists, please don't remove it, and you can develop this package normally."
  exit 0
fi
...
text_out 0 "Notice" "'../cppp-reiconv' for C++ Plus are not exists, cloning it......"
execute "git clone https://github.com/cppp-project/cppp-reiconv"
if [ $? -eq 0 ]
then
  text_out 32 "Success" "'./cppp-reiconv' cloned successfully, please don't remove it, and you can develop this package normally."
else
  text_out 31 "Error" "'./cppp-reiconv' cloned failed, you can clone it by yourself or copy it from C++ Plus release source package."
  text_out 0 "Notice" "try to run 'git clone https://github.com/cppp-project/cppp-reiconv'. "
fi
